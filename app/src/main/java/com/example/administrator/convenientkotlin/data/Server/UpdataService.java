...
public class UpdataService extends Service {
    ...
    private void initDownManager() {
        ...
        // 设置下载后文件存放的位置
        down.setDestinationInExternalPublicDir(DOWNLOADPATH, "bld.apk");
        down.setTitle("晋享其中");
        // 将下载请求放入队列
        manager.enqueue(down);
        //注册下载广播
        registerReceiver(receiver, new IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE));
        Log.i(TAG, "initDownManager: 下载开始");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Log.i(TAG, "onStartCommand: uri"+url);
        String path = Environment.getExternalStorageDirectory().getAbsolutePath() +DOWNLOADPATH+ "jxqz.apk";
    ...
    class DownloadCompleteReceiver extends BroadcastReceiver {
        ...
        private void installAPK(Uri apk, Context context) {
            if (Build.VERSION.SDK_INT < 23) {
                ...
                File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +DOWNLOADPATH+ "jxqz.apk");
                if (file.exists()) {
